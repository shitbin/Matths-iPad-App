...
for behavior in '자료실 관리' 'case files = "자료", trash = "휴지통"' '파일 선택 (최대 20개)' '등록 후 회원에게 공지' '파일 원본 영구 삭제' 'viewport.size.width >= 760' 'case .bulk(let ids, let titles)' '30일 동안 복구할 수 있습니다'; do grep -Fq "$behavior" "$screen" || { echo "FAIL missing UI $behavior" >&2; exit 1; }; done
