...
grep -Fq 'ArenaEvidenceRenderPolicy.plan(bounds: bounds, isEmpty: strokes.isEmpty)' "$match"
grep -Fq 'if isEmpty { source = base }' "$root/Matths/ArenaEvidenceRenderPolicy.swift"
grep -Fq '!bounds.isNull, !bounds.isInfinite' "$root/Matths/ArenaEvidenceRenderPolicy.swift"
grep -Fq 'pixels.width <= 2048, pixels.height <= 2048' "$root/Matths/ArenaEvidenceRenderPolicy.swift"
