...
weekly_guards=$(grep -Fc 'guard accountIsCurrent' "$weekly")
...
grep -Fq 'WeeklyMockIntegrityScreen(accountSlot: accountSlot, owner: owner)' "$weekly"
grep -Fq 'WeeklyMockObjectionScreen(accountSlot: accountSlot, owner: owner)' "$weekly"
grep -Fq 'WeeklyMockSelectionView(selection: selection, accountSlot: accountSlot, owner: owner,' "$weekly"
grep -Fq 'owner.isCurrent(in: store) && requests.isActive' "$weekly"
