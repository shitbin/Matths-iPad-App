...
grep -Fq 'LocalAIModelPack.prepareArtifact(url: url, file: file)' "$legacy"
grep -Fq 'LocalAIArtifactFlight.shared.perform(key: key)' "$pack"
grep -Fq 'ModelDownloadResponseValidation.accepts(' "$broker"
grep -Fq 'ModelIntegrityReceipt.read(for: modelURL, expectedSHA256: expectedSHA256)' "$pack"
grep -Fq 'downloadAttemptID == attemptID' "$legacy"
