...
extension AppStore {
    private var todayOfficialAttempt: AssessmentAttemptV2? {
        attemptsV2.attempts.filter { $0.serverBacked == true && $0.submittedAt == nil }
    ...
    var todayActionCandidates: [TodayActionCandidate] {
        var values: [TodayActionCandidate] = []
        if let attempt = todayOfficialAttempt {
            values.append(.init(id: attempt.id, kind: .timedWork, title: attempt.title,
                                reason: "시작한 공식 평가가 있어요. 저장한 답안에서 이어가세요.",
                                action: "평가 이어서 보기", minutes: nil))
        }
        if let attendance = todayAcademyAttendance, attendance.canCheckIn {
        ...
        if !todayReviewIDs.isEmpty {
            values.append(.init(id: "review", kind: .review, title: "오늘 복습할 오답 \(todayReviewIDs.count)개",
                                reason: "복습 예정일이 된 문제부터 다시 풀어봐요.", action: "복습 시작", minutes: nil))
        }
    ...
    var resolvedTodayAction: TodayActionCandidate? { TodayActionResolver.resolve(todayActionCandidates) }
